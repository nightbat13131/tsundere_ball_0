##https://github.com/KoBeWi/Godot-Palette-Swap-Shader/blob/master/addons/PaletteSwapShader/PaletteMaterial.gd
...
@export var palette: Texture2D #: set = set_palette


var _selected_pallet := PalletSelection.SKY

func set_pallet_selection(selection: PalletSelection) -> void:
	if selection as int != 0:
		_selected_pallet = selection
		shader = preload('uid://rgcftreb4t6t')
		apply_pallet() 
	else: 
		shader = null
...
func apply_pallet() -> void:
	...
	assert(color_count <= SHADER_ARRAY_SIZE)
	
	var pallet_row : int = _selected_pallet as int
	
	var source: PackedInt32Array
	source.resize(SHADER_ARRAY_SIZE*3)
	
	var output: PackedVector3Array
	output.resize(SHADER_ARRAY_SIZE)
	...
	for x in color_count:
		var color := image.get_pixel(x, pallet_row)
		output[x] = Vector3(color.r, color.g, color.b)
